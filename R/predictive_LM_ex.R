...
Predictive_LM_ex<-function(object,ex_data){
  ...
  Vset1$data<- Vset
  Vset1$lm_col="LM"
  Vset_i<-add_interactions(Vset1,c(cov$fixed,cov$vary),func_covars=func_covars,func_lms =func_lms,sl=sl)
  Vset_2<-Vset_i$data
  cindex<-rep(NA,nsl)
  score<-rep(NA,nsl)
  auc<-rep(NA,nsl)
  for (i in 1:nsl){
    ...
    score[i]<-cal_brierscore(model=Model,TSet$data,Vdata,width=w, tout=sl[i],time,status)
    auc[i]<-cal_auc(model=Model,data=Vdata,pred.t=sl[i]+w,time,status)
