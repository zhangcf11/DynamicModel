...
#' @param form_splines Form of the dynamic effects function.It mainly includes spline functions (spline) and quadratic functions (quadratic).
#' @param tt Follow-up time with the maximum of interest.
#' @param IDnames Label of the plot, with the default being NULL;
#' @param legend Annotations or descriptions of the plot, with the default being NULL;
#' @param xlab Label of the horizontal axis, with the default being "Follow-up Time (years)".
#' @param ylab Label of the vertical axis, with the default being "Dynamic Hazard Ratio".
...
#' @param col Line Color Options, with the default being "black".
#' @param lty Line Type Options.By default, the HR is represented by a solid line, and the confidence interval is represented by a dashed line.
#' @param lwd Line Thickness Options , with the default being "lwd=6".
...
plot_dynamicHR<-function(object,variable,form_splines,tt,IDnames=NULL,legend=NULL,xlab = "Follow-up Time (years)",ylab = "Dynamic Hazard Ratio",ylim =NULL, col = c("black", "black", "black"), lty = c(1, 2, 2),lwd=6,...){
