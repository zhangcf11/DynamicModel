...
predictive_LM<-function(object,method,n,K,seed,r){
  ...
  if (method=="CV"){
    ...
    for(i in 1:(5*n)){
      tryCatch({
        set.seed(seed+10*i )
        folds<-createFolds(unique(data[[id]]),K)
...
cal_auc<-function(model,data,pred.t,time,status){
  ...
  auc<-survivalROC(Stime=data[[time]],status=data[[status]],marker=risk,
                   predict.time = pred.t,  #ֻ  ָ  һ  ʱ
                   method="KM")
  #tt<-auc$TP-auc$FP
  #LM<-auc$TP*sum(data$status==1)
  #TY<-(1-auc$FP)*sum(data$status==0)
  #right<-(LM+TY)/nrow(data)
  #LM1<-auc$TP
  #TY1<-(1-auc$FP)
  #all<-as.data.frame(cbind(tt,auc$cut.values,LM,TY,right,LM1,TY1))
  #maxt<-max(tt)
  #all_max<-all[which(all$tt==maxt),]
  # colnames(all_max)<-c('youden','cutoff','真阳性数','真阴性数','right',' 灵敏度','特异度')
  #LM<-all[which(all$tt==maxt),3]
  #TY<-all[which(all$tt==maxt),4]
